 ...
 *   - otherwise, if `vis_p` is `HIDE_P`, must act as
 * `patomic_opsig_void_noarg_t`
 *
 * - type:
...
#define PATOMIC_WRAPPED_FETCH_DEFINE_OP_WITH_ARG(do_fetch_with_arg, type,      \
                                                 atype, fn_name, vis_p)        \
    static PATOMIC_FORCE_INLINE void fn_name(                                  \
        volatile void * obj, const void * arg vis_p(_, int order))             \
    {                                                                          \
        /* declarations */                                                     \
        type val;                                                              \
        /* assertions */                                                       \
        ...
        PATOMIC_IGNORE_UNUSED((PATOMIC_IGNORE_UNUSED(0) vis_p(                 \
            _,                                                                 \
            PATOMIC_WRAPPED_DO_ASSERT(patomic_is_valid_order((int)order)))));  \
        /* operation */                                                        \
        do_fetch_with_arg(obj, arg vis_p(_, order), &val);                     \
        PATOMIC_IGNORE_UNUSED(val);                                            \
    }

#define PATOMIC_WRAPPED_FETCH_DEFINE_OP_NOARG(do_fetch_noarg, type, atype,     \
                                              fn_name, vis_p)                  \
    static PATOMIC_FORCE_INLINE void fn_name(                                  \
        volatile void * obj vis_p(_, int order))                               \
    {                                                                          \
        /* declarations */                                                     \
        type val;                                                              \
        /* assertions */                                                       \
        ...
        PATOMIC_IGNORE_UNUSED((PATOMIC_IGNORE_UNUSED(0) vis_p(                 \
            ...
            PATOMIC_WRAPPED_DO_ASSERT(patomic_is_valid_order((int)order)))));  \
        /* operation */                                                        \
        do_fetch_noarg(obj vis_p(_, order), &val);                             \
        PATOMIC_IGNORE_UNUSED(val);                                            \
    }
