 ...
 *   - `exp`, `des`, `succ`, `fail`, and `ok` will be the names of local
 * identifiers
 ...
 *   - `r` may be uninitialised and should be set to the result of the
 * comparison
 *
 * - do_cmp_eqz:
...
#define PATOMIC_WRAPPED_CMPXCHG_DEFINE_OP_STORE(                               \
    bit_width, byte_width, do_atomic_cmpxchg_weak_explicit, type, atype,       \
    fn_name, order, vis_p)                                                     \
    static PATOMIC_FORCE_INLINE void fn_name(                                  \
        volatile void * obj, const void * desired vis_p(_, int order))         \
    {                                                                          \
        /* declarations */                                                     \
        type exp_val = {0};                                                    \
        type des_val;                                                          \
        ...
        /* operation */                                                        \
        do {                                                                   \
            do_atomic_cmpxchg_weak_explicit(type, bit_width, byte_width,       \
                                            (volatile atype *)obj, exp_val,    \
                                            des_val, succ, fail, ok);          \
        } while(!ok);                                                          \
        /* cleanup */                                                          \
...
#define PATOMIC_WRAPPED_CMPXCHG_DEFINE_OP_CMPXCHG_STRONG(                      \
    bit_width, byte_width, do_atomic_cmpxchg_weak_explicit, do_cmp_neq, type,  \
    atype, fn_name, order, vis_p, inv)                                         \
    static PATOMIC_FORCE_INLINE int fn_name(                                   \
        volatile void * obj, void * expected,                                  \
        const void * desired vis_p(_, int succ) vis_p(_, int fail))            \
    {                                                                          \
        /* declarations */                                                     \
        type exp_val;                                                          \
        ...
        inv(int succ = (int)order;)                                            \
            inv(int fail =                                                     \
                    patomic_cmpxchg_fail_order((int)order);) /* assertions */  \
            PATOMIC_WRAPPED_DO_ASSERT(obj != NULL);                            \
        ...
        do {                                                                   \
            do_atomic_cmpxchg_weak_explicit(type, bit_width, byte_width,       \
                                            (volatile atype *)obj, exp_val,    \
                                            des_val, succ, fail, ret);         \
            do_cmp_neq(type, bit_width, byte_width, exp_val, old_val, cmp);    \
            /* expected was modified */                                        \
            if(cmp)                                                            \
            {                                                                  \
                break;                                                         \
            } /* spurious fail */                                              \
            /* use memcpy instead of assignment operation */ /* in case op     \
                                                                compares       \
                                                                bitwise        \
                                                                instead of by                                                                            \
                                                                value */       \
...
#define PATOMIC_WRAPPED_CMPXCHG_DEFINE_OP_BIT_TEST_COMPL(                      \
    bit_width, byte_width, do_atomic_cmpxchg_weak_explicit, do_cmp_eqz,        \
    do_ip_nth_bit_mask, do_ip_bit_and, do_ip_bit_xor, type, atype, fn_name,    \
    order, vis_p)                                                              \
    static PATOMIC_FORCE_INLINE int fn_name(volatile void * obj,               \
                                            int offset vis_p(_, int order))    \
    {                                                                          \
        /* declarations */                                                     \
        type exp_val = {0};                                                    \
        type des_val;                                                          \
        ...
        succ = (int)order;                                                     \
        fail = patomic_cmpxchg_fail_order(succ);                               \
        do_ip_nth_bit_mask(type, bit_width, byte_width, mask, offset);         \
        /* operation */                                                        \
        do {                                                                   \
            PATOMIC_WRAPPED_DO_MEMCPY(&des_val, &exp_val, sizeof(type));       \
            do_ip_bit_xor(type, bit_width, byte_width, des_val, mask);         \
            do_atomic_cmpxchg_weak_explicit(type, bit_width, byte_width,       \
                                            (volatile atype *)obj, exp_val,    \
                                            des_val, succ, fail, ok);          \
        } while(!ok);                                                          \
        ...
        PATOMIC_IGNORE_UNUSED(scratch);                                        \
        PATOMIC_IGNORE_UNUSED(temp);                                           \
        do_ip_bit_and(type, bit_width, byte_width, exp_val, mask);             \
        do_cmp_eqz(type, bit_width, byte_width, exp_val, ok);                  \
        return !ok;                                                            \
    }

#define PATOMIC_WRAPPED_CMPXCHG_DEFINE_OP_BIT_TEST_SET(                        \
    bit_width, byte_width, do_atomic_cmpxchg_weak_explicit, do_cmp_eqz,        \
    do_ip_nth_bit_mask, do_ip_bit_and, do_ip_bit_or, type, atype, fn_name,     \
    order, vis_p)                                                              \
    static PATOMIC_FORCE_INLINE int fn_name(volatile void * obj,               \
                                            int offset vis_p(_, int order))    \
    {                                                                          \
        ...
        do {                                                                   \
            PATOMIC_WRAPPED_DO_MEMCPY(&des_val, &exp_val, sizeof(type));       \
            do_ip_bit_or(type, bit_width, byte_width, des_val, mask);          \
            do_atomic_cmpxchg_weak_explicit(type, bit_width, byte_width,       \
                                            (volatile atype *)obj, exp_val,    \
                                            des_val, succ, fail, ok);          \
        } while(!ok);                                                          \
...
#define PATOMIC_WRAPPED_CMPXCHG_DEFINE_OP_BIT_TEST_RESET(                      \
    bit_width, byte_width, do_atomic_cmpxchg_weak_explicit, do_cmp_eqz,        \
    do_ip_nth_bit_mask, do_ip_bit_and, do_ip_bit_not, type, atype, fn_name,    \
    order, vis_p)                                                              \
    static PATOMIC_FORCE_INLINE int fn_name(volatile void * obj,               \
                                            int offset vis_p(_, int order))    \
    {                                                                          \
        ...
        do_ip_nth_bit_mask(type, bit_width, byte_width, mask, offset);         \
        mask_inv = mask;                                                       \
        do_ip_bit_not(type, bit_width, byte_width, mask_inv);                  \
        /* operation */                                                        \
        do {                                                                   \
            PATOMIC_WRAPPED_DO_MEMCPY(&des_val, &exp_val, sizeof(type));       \
            do_ip_bit_and(type, bit_width, byte_width, des_val, mask_inv);     \
            do_atomic_cmpxchg_weak_explicit(type, bit_width, byte_width,       \
                                            (volatile atype *)obj, exp_val,    \
                                            des_val, succ, fail, ok);          \
        } while(!ok);                                                          \
...
#define PATOMIC_WRAPPED_CMPXCHG_DEFINE_OP_FETCH_OR(                            \
    bit_width, byte_width, do_atomic_cmpxchg_weak_explicit, do_ip_bit_or,      \
    type, atype, fn_name, order, vis_p)                                        \
    static PATOMIC_FORCE_INLINE void fn_name(                                  \
        volatile void * obj, const void * arg vis_p(_, int order), void * ret) \
    {                                                                          \
        /* declarations */                                                     \
        type exp_val = {0};                                                    \
        type des_val;                                                          \
        ...
        do {                                                                   \
            des_val = exp_val;                                                 \
            do_ip_bit_or(type, bit_width, byte_width, des_val, arg_val);       \
            do_atomic_cmpxchg_weak_explicit(type, bit_width, byte_width,       \
                                            (volatile atype *)obj, exp_val,    \
                                            des_val, succ, fail, ok);          \
        } while(!ok);                                                          \
...
#define PATOMIC_WRAPPED_CMPXCHG_DEFINE_OP_FETCH_XOR(                           \
    bit_width, byte_width, do_atomic_cmpxchg_weak_explicit, do_ip_bit_xor,     \
    type, atype, fn_name, order, vis_p)                                        \
    static PATOMIC_FORCE_INLINE void fn_name(                                  \
        volatile void * obj, const void * arg vis_p(_, int order), void * ret) \
    {                                                                          \
        ...
        do {                                                                   \
            des_val = exp_val;                                                 \
            do_ip_bit_xor(type, bit_width, byte_width, des_val, arg_val);      \
            do_atomic_cmpxchg_weak_explicit(type, bit_width, byte_width,       \
                                            (volatile atype *)obj, exp_val,    \
                                            des_val, succ, fail, ok);          \
        } while(!ok);                                                          \
...
#define PATOMIC_WRAPPED_CMPXCHG_DEFINE_OP_FETCH_AND(                           \
    bit_width, byte_width, do_atomic_cmpxchg_weak_explicit, do_ip_bit_and,     \
    type, atype, fn_name, order, vis_p)                                        \
    static PATOMIC_FORCE_INLINE void fn_name(                                  \
        volatile void * obj, const void * arg vis_p(_, int order), void * ret) \
    {                                                                          \
        ...
        do {                                                                   \
            des_val = exp_val;                                                 \
            do_ip_bit_and(type, bit_width, byte_width, des_val, arg_val);      \
            do_atomic_cmpxchg_weak_explicit(type, bit_width, byte_width,       \
                                            (volatile atype *)obj, exp_val,    \
                                            des_val, succ, fail, ok);          \
        } while(!ok);                                                          \
...
#define PATOMIC_WRAPPED_CMPXCHG_DEFINE_OP_FETCH_NOT(                           \
    bit_width, byte_width, do_atomic_cmpxchg_weak_explicit, do_ip_bit_not,     \
    type, atype, fn_name, order, vis_p)                                        \
    static PATOMIC_FORCE_INLINE void fn_name(                                  \
        volatile void * obj vis_p(_, int order), void * ret)                   \
    {                                                                          \
        /* declarations */                                                     \
        type exp_val = {0};                                                    \
        type des_val;                                                          \
        ...
        do {                                                                   \
            des_val = exp_val;                                                 \
            do_ip_bit_not(type, bit_width, byte_width, des_val);               \
            do_atomic_cmpxchg_weak_explicit(type, bit_width, byte_width,       \
                                            (volatile atype *)obj, exp_val,    \
                                            des_val, succ, fail, ok);          \
        } while(!ok);                                                          \
...
#define PATOMIC_WRAPPED_CMPXCHG_DEFINE_OP_FETCH_ADD(                           \
    bit_width, byte_width, do_atomic_cmpxchg_weak_explicit, do_ip_add, type,   \
    atype, fn_name, order, vis_p, min_val)                                     \
    static PATOMIC_FORCE_INLINE void fn_name(                                  \
        volatile void * obj, const void * arg vis_p(_, int order), void * ret) \
    {                                                                          \
        /* declarations */                                                     \
        ...
        do {                                                                   \
            des_val = exp_val;                                                 \
            do_ip_add(type, bit_width, byte_width, min_val, des_val, arg_val); \
            do_atomic_cmpxchg_weak_explicit(type, bit_width, byte_width,       \
                                            (volatile atype *)obj, exp_val,    \
                                            des_val, succ, fail, ok);          \
        } while(!ok);                                                          \
...
#define PATOMIC_WRAPPED_CMPXCHG_DEFINE_OP_FETCH_SUB(                           \
    bit_width, byte_width, do_atomic_cmpxchg_weak_explicit, do_ip_sub, type,   \
    atype, fn_name, order, vis_p, min_val)                                     \
    static PATOMIC_FORCE_INLINE void fn_name(                                  \
        volatile void * obj, const void * arg vis_p(_, int order), void * ret) \
    {                                                                          \
        ...
        do {                                                                   \
            des_val = exp_val;                                                 \
            do_ip_sub(type, bit_width, byte_width, min_val, des_val, arg_val); \
            do_atomic_cmpxchg_weak_explicit(type, bit_width, byte_width,       \
                                            (volatile atype *)obj, exp_val,    \
                                            des_val, succ, fail, ok);          \
        } while(!ok);                                                          \
...
#define PATOMIC_WRAPPED_CMPXCHG_DEFINE_OP_FETCH_INC(                           \
    bit_width, byte_width, do_atomic_cmpxchg_weak_explicit, do_ip_inc, type,   \
    atype, fn_name, order, vis_p, min_val)                                     \
    static PATOMIC_FORCE_INLINE void fn_name(                                  \
        volatile void * obj vis_p(_, int order), void * ret)                   \
    {                                                                          \
        ...
        do {                                                                   \
            des_val = exp_val;                                                 \
            do_ip_inc(type, bit_width, byte_width, min_val, des_val);          \
            do_atomic_cmpxchg_weak_explicit(type, bit_width, byte_width,       \
                                            (volatile atype *)obj, exp_val,    \
                                            des_val, succ, fail, ok);          \
        } while(!ok);                                                          \
...
#define PATOMIC_WRAPPED_CMPXCHG_DEFINE_OP_FETCH_DEC(                           \
    bit_width, byte_width, do_atomic_cmpxchg_weak_explicit, do_ip_dec, type,   \
    atype, fn_name, order, vis_p, min_val)                                     \
    static PATOMIC_FORCE_INLINE void fn_name(                                  \
        volatile void * obj vis_p(_, int order), void * ret)                   \
    {                                                                          \
        ...
        do {                                                                   \
            des_val = exp_val;                                                 \
            do_ip_dec(type, bit_width, byte_width, min_val, des_val);          \
            do_atomic_cmpxchg_weak_explicit(type, bit_width, byte_width,       \
                                            (volatile atype *)obj, exp_val,    \
                                            des_val, succ, fail, ok);          \
        } while(!ok);                                                          \
...
#define PATOMIC_WRAPPED_CMPXCHG_DEFINE_OP_FETCH_NEG(                           \
    bit_width, byte_width, do_atomic_cmpxchg_weak_explicit, do_ip_neg, type,   \
    atype, fn_name, order, vis_p, min_val)                                     \
    static PATOMIC_FORCE_INLINE void fn_name(                                  \
        volatile void * obj vis_p(_, int order), void * ret)                   \
    {                                                                          \
        ...
        do {                                                                   \
            des_val = exp_val;                                                 \
            do_ip_neg(type, bit_width, byte_width, min_val, des_val);          \
            do_atomic_cmpxchg_weak_explicit(type, bit_width, byte_width,       \
                                            (volatile atype *)obj, exp_val,    \
                                            des_val, succ, fail, ok);          \
        } while(!ok);                                                          \
