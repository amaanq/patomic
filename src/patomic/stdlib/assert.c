...
static void
patomic_assert_fxprint(FILE * stream, const char * expr, const char * file,
                       const char * func, unsigned int line)
{
#if PATOMIC_ASSERT_WIDE
    if(fwide(stream, -1) > 0)
    {
        PATOMIC_IGNORE_UNUSED(fwprintf(stream,
                                       L"%s:%d: %s: Assertion `%s` failed.\n",
                                       file, line, func, expr));
    }
    else
    {
#endif
        PATOMIC_IGNORE_UNUSED(fprintf(stream,
...
void
__patomic_assert_fail(const char * expr, const char * file, const char * func,
                      unsigned int line)
{
    int _;
