...
#define PATOMIC_UNSET_OPCAT_LDST(ops, cats, and_or)                            \
    do {                                                                       \
        if((cats & patomic_opcat_LDST) &&                                      \
           ((ops->fp_load != NULL) and_or(ops->fp_store != NULL)))             \
        {                                                                      \
...
#define PATOMIC_UNSET_OPCAT_XCHG(ops, cats, and_or)                            \
    do {                                                                       \
        if((cats & patomic_opcat_XCHG) &&                                      \
           ((ops->xchg_ops.fp_exchange != NULL)                                \
                and_or(ops->xchg_ops.fp_cmpxchg_weak != NULL)                  \
                    and_or(ops->xchg_ops.fp_cmpxchg_strong != NULL)))          \
        {                                                                      \
...
#define PATOMIC_UNSET_OPCAT_BIT(ops, cats, and_or)                             \
    do {                                                                       \
        if((cats & patomic_opcat_BIT) &&                                       \
           ((ops->bitwise_ops.fp_test != NULL)                                 \
                and_or(ops->bitwise_ops.fp_test_set != NULL)                   \
                    and_or(ops->bitwise_ops.fp_test_reset != NULL)             \
                        and_or(ops->bitwise_ops.fp_test_compl != NULL)))       \
        {                                                                      \
...
#define PATOMIC_UNSET_OPCAT_BIN(ops, cats, and_or, vf, fp_sep)                 \
    do {                                                                       \
        if((cats & patomic_opcat_BIN_##vf) &&                                  \
           ((ops->binary_ops.fp##fp_sep## or != NULL)                          \
                and_or(ops->binary_ops.fp##fp_sep## xor != NULL)               \
                    and_or(ops->binary_ops.fp##fp_sep## and != NULL)           \
                        and_or(ops->binary_ops.fp##fp_sep##not != NULL)))      \
        {                                                                      \
    ...
    do {                                                                       \
        if((cats & patomic_opcat_##su##ARI_##vf) &&                            \
           ((ops->uns_pfx##igned_ops.fp##fp_sep##add != NULL) and_or(          \
               ops->uns_pfx##igned_ops.fp##fp_sep##sub !=                      \
               NULL) and_or(ops->uns_pfx##igned_ops.fp##fp_sep##inc != NULL)   \
                and_or(ops->uns_pfx##igned_ops.fp##fp_sep##dec != NULL)        \
                    and_or(ops->uns_pfx##igned_ops.fp##fp_sep##neg != NULL)))  \
        {                                                                      \
...
#define PATOMIC_UNSET_OPCAT_SPEC(ops, cats, and_or)                            \
    do {                                                                       \
        if((cats & patomic_opcat_SPEC) &&                                      \
           ((ops->special_ops.fp_double_cmpxchg != NULL)                       \
                and_or(ops->special_ops.fp_multi_cmpxchg != NULL)              \
                    and_or(ops->special_ops.fp_generic != NULL)                \
                        and_or(ops->special_ops.fp_generic_wfb != NULL)))      \
        {                                                                      \
...
#define PATOMIC_UNSET_OPCAT_FLAG(ops, cats, and_or)                            \
    do {                                                                       \
        if((cats & patomic_opcat_FLAG) &&                                      \
           ((ops->flag_ops.fp_test != NULL)                                    \
                and_or(ops->flag_ops.fp_test_set != NULL)                      \
                    and_or(ops->flag_ops.fp_clear)))                           \
        {                                                                      \
...
#define PATOMIC_UNSET_OPCAT_RAW(ops, cats, and_or)                             \
    do {                                                                       \
        if((cats & patomic_opcat_RAW) &&                                       \
           ((ops->raw_ops.fp_tbegin != NULL)                                   \
                and_or(ops->raw_ops.fp_tabort != NULL)                         \
                    and_or(ops->raw_ops.fp_tcommit != NULL)                    \
                        and_or(ops->raw_ops.fp_ttest != NULL)))                \
        {                                                                      \
...
unsigned int
patomic_feature_check_all(patomic_ops_t const * const ops, unsigned int opcats)
{
    if(opcats == 0u)
    {
        return opcats;
...
unsigned int
patomic_feature_check_all_explicit(patomic_ops_explicit_t const * const ops,
                                   unsigned int opcats)
{
    if(opcats == 0u)
    {
...
patomic_feature_check_all_transaction(
    patomic_ops_transaction_t const * const ops, unsigned int opcats)
{
    if(opcats == 0u)
    {
        return opcats;
...
unsigned int
patomic_feature_check_any(patomic_ops_t const * const ops, unsigned int opcats)
{
    if(opcats == 0u)
    {
        return opcats;
...
unsigned int
patomic_feature_check_any_explicit(patomic_ops_explicit_t const * const ops,
                                   unsigned int opcats)
{
    if(opcats == 0u)
    {
...
#define PATOMIC_UNSET_OPKINDS_XCHG(ops, kinds)                                 \
    ...
    PATOMIC_CHK_UNSET_OPKIND(ops->xchg_ops.fp_cmpxchg_weak, kinds,             \
                             CMPXCHG_WEAK);                                    \
    PATOMIC_CHK_UNSET_OPKIND(ops->xchg_ops.fp_cmpxchg_strong, kinds,           \
                             CMPXCHG_STRONG)
...
#define PATOMIC_UNSET_OPKINDS_BIT(ops, kinds)                                  \
    ...
    PATOMIC_CHK_UNSET_OPKIND(ops->bitwise_ops.fp_test_reset, kinds,            \
                             TEST_RESET);                                      \
...
#define PATOMIC_UNSET_OPKINDS_SPEC(ops, kinds)                                 \
    PATOMIC_CHK_UNSET_OPKIND(ops->special_ops.fp_double_cmpxchg, kinds,        \
                             DOUBLE_CMPXCHG);                                  \
    PATOMIC_CHK_UNSET_OPKIND(ops->special_ops.fp_multi_cmpxchg, kinds,         \
                             MULTI_CMPXCHG);                                   \
    PATOMIC_CHK_UNSET_OPKIND(ops->special_ops.fp_generic, kinds, GENERIC);     \
    PATOMIC_CHK_UNSET_OPKIND(ops->special_ops.fp_generic_wfb, kinds,           \
                             GENERIC_WFB)
...
#define PATOMIC_CASE_UNSET_OPKINDS(cat, ops, kinds)                            \
    case patomic_opcat_##cat: PATOMIC_UNSET_OPKINDS_##cat(ops, opkinds); break

PATOMIC_FORCE_INLINE static int
patomic_is_pow2_or_zero(unsigned int val)
{
    return (val & (val - 1)) == 0;
}

unsigned int
patomic_feature_check_leaf(patomic_ops_t const * const ops, unsigned int opcat,
                           unsigned int opkinds)
{
    if((opcat == 0u) || (opkinds == 0u))
    {
        return opkinds;
    }
...
unsigned int
patomic_feature_check_leaf_explicit(patomic_ops_explicit_t const * const ops,
                                    unsigned int opcat, unsigned int opkinds)
{
    if((opcat == 0u) || (opkinds == 0u))
    {
        return opkinds;
...
patomic_feature_check_leaf_transaction(
    patomic_ops_transaction_t const * const ops, unsigned int opcat,
    unsigned int opkinds)
{
    if((opcat == 0u) || (opkinds == 0u))
    {
