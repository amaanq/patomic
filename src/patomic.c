...
void
patomic_combine(patomic_t * const priority, patomic_t const * const other)
{
    int i = 0;
    PATOMIC_CCOPY_IE_OPS(priority, other, i);
    if(i != 0)
    {
        PATOMIC_COPY_ALIGN(priority, other);
    }
}

void
patomic_combine_explicit(patomic_explicit_t * const priority,
...
patomic_t
patomic_create(size_t byte_width, patomic_memory_order_t order,
               unsigned int opts, unsigned int kinds, unsigned long ids)
{
    ...
    for(i = 0; i < PATOMIC_IMPL_REGISTER_SIZE; ++i)
    {
        ...
        {
            *end++ =
                patomic_impl_register[i].fp_create(byte_width, order, opts);
        }
    }
                        ...
                        unsigned int kinds, unsigned long ids)
{
    ...
    for(i = 0; i < PATOMIC_IMPL_REGISTER_SIZE; ++i)
    {
        ...
        {
            *end++ =
                patomic_impl_register[i].fp_create_explicit(byte_width, opts);
        }
    }
...
patomic_create_transaction(unsigned int opts, unsigned int kinds,
                           unsigned long ids)
{
    ...
    for(; begin != end; ++begin)
    {
        ...
        if(ret.ops.raw_ops.fp_tbegin != NULL)
        {
            return ret;
        }
    }
