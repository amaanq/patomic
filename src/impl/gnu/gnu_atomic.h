...
#define PATOMIC_DEFINE_STORE_OPS(type, name, order, vis_p)                     \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_STORE(0, 0, patomic_do_store_e, type,     \
                                           type, patomic_opimpl_store_##name,  \
                                           order, vis_p)

#define PATOMIC_DEFINE_LOAD_OPS(type, name, order, vis_p)                      \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_LOAD(0, 0, patomic_do_load_e, type, type, \
                                          patomic_opimpl_load_##name, order,   \
                                          vis_p)
 ...
 */
#define patomic_do_exchange_e(t, i, y, p, d, o, r)                             \
    r = __atomic_exchange_n(p, d, o)
...
#define PATOMIC_DEFINE_BIT_TEST_OPS(type, name, order, vis_p)                  \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_BIT_TEST(                                 \
        0, 0, patomic_do_bt_e, type, type, patomic_opimpl_bit_test_##name,     \
        order, vis_p)
...
#define PATOMIC_DEFINE_BIN_OPS_CREATE(type, name, order, vis_p, opsk)          \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_FETCH_OR(                                 \
        0, 0, patomic_do_or_fe, type, type, patomic_opimpl_fetch_or_##name,    \
        order, vis_p)                                                          \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_FETCH_XOR(                                \
        0, 0, patomic_do_xor_fe, type, type, patomic_opimpl_fetch_xor_##name,  \
        order, vis_p)                                                          \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_FETCH_AND(                                \
        0, 0, patomic_do_and_fe, type, type, patomic_opimpl_fetch_and_##name,  \
        order, vis_p)                                                          \
    PATOMIC_WRAPPED_CMPXCHG_DEFINE_OP_FETCH_NOT(                               \
        0, 0, patomic_do_cmpxchg_we, patomic_do_ip_bit_not, type, type,        \
        patomic_opimpl_fetch_not_##name, order, vis_p)                         \
...
#define PATOMIC_DEFINE_ARI_OPS_CREATE(type, name, order, vis_p, opsk, min)     \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_FETCH_ADD(                                \
        0, 0, patomic_do_add_fe, type, type, patomic_opimpl_fetch_add_##name,  \
        order, vis_p, min)                                                     \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_FETCH_SUB(                                \
        0, 0, patomic_do_sub_fe, type, type, patomic_opimpl_fetch_sub_##name,  \
        order, vis_p, min)                                                     \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_FETCH_INC(                                \
        0, 0, patomic_do_inc_fe, type, type, patomic_opimpl_fetch_inc_##name,  \
        order, vis_p, min)                                                     \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_FETCH_DEC(                                \
        0, 0, patomic_do_dec_fe, type, type, patomic_opimpl_fetch_dec_##name,  \
        order, vis_p, min)                                                     \
    PATOMIC_WRAPPED_CMPXCHG_DEFINE_OP_FETCH_NEG(                               \
        0, 0, patomic_do_cmpxchg_we, patomic_do_ip_neg, type, type,            \
...
#define PATOMIC_DEFINE_IS_LOCK_FREE(type, name)                                \
    static PATOMIC_FORCE_INLINE int patomic_impl_is_lock_free_##name(void)     \
    {                                                                          \
...
#define PATOMIC_DEFINE_OPS_CREATE_ALL(type, name, min)                         \
    PATOMIC_DEFINE_OPS_CREATE_CA(type, name##_acquire, __ATOMIC_ACQUIRE,       \
                                 HIDE_P, SHOW, ops, min)                       \
    PATOMIC_DEFINE_OPS_CREATE_R(type, name##_release, __ATOMIC_RELEASE,        \
                                HIDE_P, SHOW, ops, min)                        \
    PATOMIC_DEFINE_OPS_CREATE_AR(type, name##_acq_rel, __ATOMIC_ACQ_REL,       \
                                 HIDE_P, SHOW, ops, min)                       \
    PATOMIC_DEFINE_OPS_CREATE_RSC(type, name##_relaxed, __ATOMIC_RELAXED,      \
                                  HIDE_P, SHOW, ops, min)                      \
    PATOMIC_DEFINE_OPS_CREATE_RSC(type, name##_seq_cst, __ATOMIC_SEQ_CST,      \
                                  HIDE_P, SHOW, ops, min)                      \
    /* all ops are in RSC */                                                   \
    PATOMIC_DEFINE_OPS_CREATE_RSC(type, name##_explicit, order, SHOW_P, HIDE,  \
                                  ops_explicit, min)                           \
    PATOMIC_DEFINE_IS_LOCK_FREE(type, name)
    ...
    if PATOMIC_SET_RET(char, char, byte_width, order, ret)
            ...
            ret) else if PATOMIC_SET_RET(int, int, byte_width, order,
                                         ret) else if PATOMIC_SET_RET(long,
                                                                      ...
                                                                      ret)
#if PATOMIC_IMPL_GNU_HAVE_LLONG
            else if PATOMIC_SET_RET(patomic_llong_signed, llong, byte_width,
                                    order, ret)
#endif
#if PATOMIC_IMPL_GNU_HAVE_I128
                else if PATOMIC_SET_RET(patomic_i128_signed, 128, byte_width,
                                        order, ret)
#endif
...
#if PATOMIC_IMPL_GNU_HAVE_LLONG
            else if PATOMIC_SET_RET_EXPLICIT(patomic_llong_signed, llong,
                                             byte_width, ret)
#endif
#if PATOMIC_IMPL_GNU_HAVE_I128
                else if PATOMIC_SET_RET_EXPLICIT(patomic_i128_signed, 128,
                                                 byte_width, ret)
#endif
...
patomic_create_align(size_t byte_width)
{
    ...
    if PATOMIC_SET_ALIGN(char, byte_width, ret.recommended)
            ...
            ret.recommended) else if PATOMIC_SET_ALIGN(long, byte_width,
                                                       ret.recommended)
#if PATOMIC_IMPL_GNU_HAVE_LLONG
            else if PATOMIC_SET_ALIGN(patomic_llong_signed, byte_width,
                                      ret.recommended)
#endif
#if PATOMIC_IMPL_GNU_HAVE_I128
                else if PATOMIC_SET_ALIGN(patomic_i128_signed, byte_width,
                                          ret.recommended)
#endif
                    else
        {
