...
#define PATOMIC_DEFINE_STORE_OPS(type, name, order, vis_p)                     \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_STORE(                                    \
        0, 0, patomic_do_store_e, type, _Atomic(type),                         \
        patomic_opimpl_store_##name, order, vis_p)

#define PATOMIC_DEFINE_LOAD_OPS(type, name, order, vis_p)                      \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_LOAD(                                     \
        0, 0, patomic_do_load_e, type, _Atomic(type),                          \
        patomic_opimpl_load_##name, order, vis_p)
 ...
 */
#define patomic_do_exchange_e(t, i, b, p, d, o, r)                             \
    r = atomic_exchange_explicit(p, d, o)
...
#define PATOMIC_DEFINE_XCHG_OPS_CREATE(type, name, order, vis_p, inv, opsk)    \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_EXCHANGE(                                 \
        0, 0, patomic_do_exchange_e, type, _Atomic(type),                      \
        patomic_opimpl_exchange_##name, order, vis_p)                          \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_CMPXCHG_WEAK(                             \
        0, 0, patomic_do_cmpxchg_we, type, _Atomic(type),                      \
        patomic_opimpl_cmpxchg_weak_##name, order, vis_p, inv)                 \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_CMPXCHG_STRONG(                           \
        0, 0, patomic_do_cmpxchg_se, type, _Atomic(type),                      \
        patomic_opimpl_cmpxchg_strong_##name, order, vis_p, inv)               \
    static patomic_##opsk##_xchg_t patomic_ops_xchg_create_##name(void)        \
    {                                                                          \
...
#define patomic_do_bt_e(t, i, y, p, x, o, r)                                   \
    scratch = (t)((t)1u << (t)(x));                                            \
    scratch &= atomic_load_explicit(p, o);                                     \
    r = (scratch != (t)0)
...
#define PATOMIC_DEFINE_BIT_TEST_OPS(type, name, order, vis_p)                  \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_BIT_TEST(                                 \
        0, 0, patomic_do_bt_e, type, _Atomic(type),                            \
        patomic_opimpl_bit_test_##name, order, vis_p)
...
#define PATOMIC_DEFINE_BIT_TEST_MODIFY_OPS(type, name, order, vis_p)           \
    PATOMIC_WRAPPED_CMPXCHG_DEFINE_OP_BIT_TEST_COMPL(                          \
        ...
        patomic_do_ip_nth_bit_mask, patomic_do_ip_bit_and,                     \
        patomic_do_ip_bit_xor, type, _Atomic(type),                            \
        patomic_opimpl_bit_test_compl_##name, order, vis_p)                    \
    PATOMIC_WRAPPED_CMPXCHG_DEFINE_OP_BIT_TEST_SET(                            \
        0, 0, patomic_do_cmpxchg_we, patomic_do_cmp_eqz,                       \
        patomic_do_ip_nth_bit_mask, patomic_do_ip_bit_and,                     \
        patomic_do_ip_bit_or, type, _Atomic(type),                             \
        patomic_opimpl_bit_test_set_##name, order, vis_p)                      \
    PATOMIC_WRAPPED_CMPXCHG_DEFINE_OP_BIT_TEST_RESET(                          \
        0, 0, patomic_do_cmpxchg_we, patomic_do_cmp_eqz,                       \
        patomic_do_ip_nth_bit_mask, patomic_do_ip_bit_and,                     \
        patomic_do_ip_bit_not, type, _Atomic(type),                            \
        patomic_opimpl_bit_test_reset_##name, order, vis_p)
...
#define PATOMIC_DEFINE_BIT_OPS_CREATE_ANY(type, name, order, vis_p, opsk)      \
    ...
    static patomic_##opsk##_bitwise_t patomic_ops_bitwise_create_##name(void)  \
    {                                                                          \
 ...
 * - (fetch_)not (y)
 */
#define patomic_do_or_fe(t, i, y, p, a, o, r)                                  \
    r = atomic_fetch_or_explicit(p, a, o)
#define patomic_do_xor_fe(t, i, y, p, a, o, r)                                 \
    r = atomic_fetch_xor_explicit(p, a, o)
#define patomic_do_and_fe(t, i, y, p, a, o, r)                                 \
    r = atomic_fetch_and_explicit(p, a, o)

#define PATOMIC_DEFINE_BIN_OPS_CREATE(type, name, order, vis_p, opsk)          \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_FETCH_OR(                                 \
        0, 0, patomic_do_or_fe, type, _Atomic(type),                           \
        patomic_opimpl_fetch_or_##name, order, vis_p)                          \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_FETCH_XOR(                                \
        0, 0, patomic_do_xor_fe, type, _Atomic(type),                          \
        patomic_opimpl_fetch_xor_##name, order, vis_p)                         \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_FETCH_AND(                                \
        0, 0, patomic_do_and_fe, type, _Atomic(type),                          \
        patomic_opimpl_fetch_and_##name, order, vis_p)                         \
    PATOMIC_WRAPPED_CMPXCHG_DEFINE_OP_FETCH_NOT(                               \
        0, 0, patomic_do_cmpxchg_we, patomic_do_ip_bit_not, type,              \
        _Atomic(type), patomic_opimpl_fetch_not_##name, order, vis_p)          \
    PATOMIC_WRAPPED_FETCH_DEFINE_OP_WITH_ARG(patomic_opimpl_fetch_or_##name,   \
                                             type, _Atomic(type),              \
                                             patomic_opimpl_or_##name, vis_p)  \
    PATOMIC_WRAPPED_FETCH_DEFINE_OP_WITH_ARG(patomic_opimpl_fetch_xor_##name,  \
                                             type, _Atomic(type),              \
                                             patomic_opimpl_xor_##name, vis_p) \
    PATOMIC_WRAPPED_FETCH_DEFINE_OP_WITH_ARG(patomic_opimpl_fetch_and_##name,  \
                                             type, _Atomic(type),              \
                                             patomic_opimpl_and_##name, vis_p) \
    PATOMIC_WRAPPED_FETCH_DEFINE_OP_NOARG(patomic_opimpl_fetch_not_##name,     \
                                          type, _Atomic(type),                 \
                                          patomic_opimpl_not_##name, vis_p)    \
    static patomic_##opsk##_binary_t patomic_ops_binary_create_##name(void)    \
    {                                                                          \
 ...
 * - (fetch_)neg (y)
 */
#define patomic_do_add_fe(t, i, y, m, p, a, o, r)                              \
    r = atomic_fetch_add_explicit(p, a, o)
#define patomic_do_sub_fe(t, i, y, m, p, a, o, r)                              \
    r = atomic_fetch_sub_explicit(p, a, o)
#define patomic_do_inc_fe(t, i, y, m, p, o, r)                                 \
    r = atomic_fetch_add_explicit(p, (t)1u, o)
#define patomic_do_dec_fe(t, i, y, m, p, o, r)                                 \
    r = atomic_fetch_sub_explicit(p, (t)1u, o)
...
#define PATOMIC_DEFINE_ARI_OPS_CREATE(type, name, order, vis_p, opsk, min)     \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_FETCH_ADD(                                \
        0, 0, patomic_do_add_fe, type, _Atomic(type),                          \
        patomic_opimpl_fetch_add_##name, order, vis_p, min)                    \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_FETCH_SUB(                                \
        0, 0, patomic_do_sub_fe, type, _Atomic(type),                          \
        patomic_opimpl_fetch_sub_##name, order, vis_p, min)                    \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_FETCH_INC(                                \
        0, 0, patomic_do_inc_fe, type, _Atomic(type),                          \
        patomic_opimpl_fetch_inc_##name, order, vis_p, min)                    \
    PATOMIC_WRAPPED_DIRECT_DEFINE_OP_FETCH_DEC(                                \
        0, 0, patomic_do_dec_fe, type, _Atomic(type),                          \
        patomic_opimpl_fetch_dec_##name, order, vis_p, min)                    \
    PATOMIC_WRAPPED_CMPXCHG_DEFINE_OP_FETCH_NEG(                               \
        0, 0, patomic_do_cmpxchg_we, patomic_do_ip_neg, type, _Atomic(type),   \
        patomic_opimpl_fetch_neg_##name, order, vis_p, min)                    \
    PATOMIC_WRAPPED_FETCH_DEFINE_OP_WITH_ARG(patomic_opimpl_fetch_add_##name,  \
                                             type, _Atomic(type),              \
                                             patomic_opimpl_add_##name, vis_p) \
    PATOMIC_WRAPPED_FETCH_DEFINE_OP_WITH_ARG(patomic_opimpl_fetch_sub_##name,  \
                                             type, _Atomic(type),              \
                                             patomic_opimpl_sub_##name, vis_p) \
    PATOMIC_WRAPPED_FETCH_DEFINE_OP_NOARG(patomic_opimpl_fetch_inc_##name,     \
                                          type, _Atomic(type),                 \
                                          patomic_opimpl_inc_##name, vis_p)    \
    PATOMIC_WRAPPED_FETCH_DEFINE_OP_NOARG(patomic_opimpl_fetch_dec_##name,     \
                                          type, _Atomic(type),                 \
                                          patomic_opimpl_dec_##name, vis_p)    \
    PATOMIC_WRAPPED_FETCH_DEFINE_OP_NOARG(patomic_opimpl_fetch_neg_##name,     \
                                          type, _Atomic(type),                 \
                                          patomic_opimpl_neg_##name, vis_p)    \
    static patomic_##opsk##_arithmetic_t patomic_ops_arithmetic_create_##name( \
        void)                                                                  \
    {                                                                          \
...
#define PATOMIC_DEFINE_OPS_CREATE_CA(type, name, order, vis_p, inv, opsk, min) \
    ...
    PATOMIC_DEFINE_XCHG_OPS_CREATE(type##_unsigned, u##name, order, vis_p,     \
                                   inv, opsk)                                  \
    PATOMIC_DEFINE_BIT_OPS_CREATE_NRAR(type##_unsigned, u##name, order, vis_p, \
                                       opsk)                                   \
    PATOMIC_DEFINE_BIN_OPS_CREATE(type##_unsigned, u##name, order, vis_p,      \
                                  opsk)                                        \
    PATOMIC_DEFINE_ARI_OPS_CREATE(type##_unsigned, u##name, order, vis_p,      \
                                  opsk, 0u)                                    \
    PATOMIC_DEFINE_ARI_OPS_CREATE(type##_signed, s##name, order, vis_p, opsk,  \
                                  min)                                         \
    static patomic_##opsk##_t patomic_ops_create_##name(void)                  \
    {                                                                          \
...
#define PATOMIC_DEFINE_OPS_CREATE_R(type, name, order, vis_p, inv, opsk, min)  \
    ...
    PATOMIC_DEFINE_XCHG_OPS_CREATE(type##_unsigned, u##name, order, vis_p,     \
                                   inv, opsk)                                  \
    PATOMIC_DEFINE_BIT_OPS_CREATE_ANY(type##_unsigned, u##name, order, vis_p,  \
                                      opsk)                                    \
    PATOMIC_DEFINE_BIN_OPS_CREATE(type##_unsigned, u##name, order, vis_p,      \
                                  opsk)                                        \
    PATOMIC_DEFINE_ARI_OPS_CREATE(type##_unsigned, u##name, order, vis_p,      \
...
#define PATOMIC_DEFINE_OPS_CREATE_RSC(type, name, order, vis_p, inv, opsk,     \
                                      min)                                     \
    PATOMIC_DEFINE_STORE_OPS(type##_unsigned, u##name, order, vis_p)           \
    PATOMIC_DEFINE_LOAD_OPS(type##_unsigned, u##name, order, vis_p)            \
    PATOMIC_DEFINE_XCHG_OPS_CREATE(type##_unsigned, u##name, order, vis_p,     \
...
#define PATOMIC_DEFINE_OPS_CREATE_ALL(type, name, min)                         \
    PATOMIC_DEFINE_OPS_CREATE_CA(type, name##_acquire, memory_order_acquire,   \
                                 HIDE_P, SHOW, ops, min)                       \
    PATOMIC_DEFINE_OPS_CREATE_R(type, name##_release, memory_order_release,    \
                                HIDE_P, SHOW, ops, min)                        \
    PATOMIC_DEFINE_OPS_CREATE_AR(type, name##_acq_rel, memory_order_acq_rel,   \
                                 HIDE_P, SHOW, ops, min)                       \
    PATOMIC_DEFINE_OPS_CREATE_RSC(type, name##_relaxed, memory_order_relaxed,  \
                                  HIDE_P, SHOW, ops, min)                      \
    PATOMIC_DEFINE_OPS_CREATE_RSC(type, name##_seq_cst, memory_order_seq_cst,  \
                                  HIDE_P, SHOW, ops, min)                      \
    /* all ops are in RSC */                                                   \
    PATOMIC_DEFINE_OPS_CREATE_RSC(type, name##_explicit, order, SHOW_P, HIDE,  \
                                  ops_explicit, min)
...
#define PATOMIC_SET_RET(type, name, width, order, ret)                         \
    (((width) == sizeof(type)) && ((width) == sizeof(_Atomic(type))))          \
    {                                                                          \
        _Atomic(type) obj;                                                     \
        ...
        {                                                                      \
            ...
            case patomic_SEQ_CST:                                              \
                ...
                break;                                                         \
            default: patomic_assert_always("invalid memory order" && 0);       \
            }                                                                  \
        }                                                                      \
...
patomic_create_ops(size_t byte_width, patomic_memory_order_t order)
{
    patomic_ops_t ret = {0};
    ...
    if PATOMIC_SET_RET(char, char, byte_width, order, ret)
            ...
            short, short, byte_width, order,
            ret) else if PATOMIC_SET_RET(int, int, byte_width, order,
                                         ret) else if PATOMIC_SET_RET(long,
                                                                      long,
                                                                      byte_width,
                                                                      order,
                                                                      ret)
#if ATOMIC_LLONG_LOCK_FREE && PATOMIC_IMPL_STD_HAVE_LLONG
            else if PATOMIC_SET_RET(patomic_llong_unsigned, llong, byte_width,
                                    order, ret)
#endif
...
patomic_create_ops_explicit(size_t byte_width)
{
    ...
    if PATOMIC_SET_RET_EXPLICIT(char, char, byte_width, ret)
        else if PATOMIC_SET_RET_EXPLICIT(short, short, byte_width, ret) else if PATOMIC_SET_RET_EXPLICIT(
            int, int, byte_width,
            ret) else if PATOMIC_SET_RET_EXPLICIT(long, long, byte_width, ret)
#if ATOMIC_LLONG_LOCK_FREE && PATOMIC_IMPL_STD_HAVE_LLONG
            else if PATOMIC_SET_RET_EXPLICIT(patomic_llong_unsigned, llong,
                                             byte_width, ret)
#endif
...
patomic_create_align(size_t byte_width)
{
    patomic_align_t ret = {0};

    if PATOMIC_SET_ALIGN(char, byte_width, ret.recommended)
        else if PATOMIC_SET_ALIGN(short, byte_width, ret.recommended) else if PATOMIC_SET_ALIGN(
            int, byte_width,
            ret.recommended) else if PATOMIC_SET_ALIGN(long, byte_width,
                                                       ret.recommended)
#if ATOMIC_LLONG_LOCK_FREE && PATOMIC_IMPL_STD_HAVE_LLONG
            else if PATOMIC_SET_ALIGN(patomic_llong_unsigned, byte_width,
                                      ret.recommended)
#endif
                else
        {
...
patomic_t
patomic_impl_create_std(size_t byte_width, patomic_memory_order_t order,
...
patomic_explicit_t
patomic_impl_create_explicit_std(size_t byte_width, unsigned int options)
{
...
patomic_impl_create_transaction_std(unsigned int options)
{
