...
#define PATOMIC_DEFINE_STORE_OPS(width, name, order, vis)                      \
    static PATOMIC_FORCE_INLINE void patomic_opimpl_store_##name(              \
        volatile void * obj, const void * desired vis(_, int order))           \
    {                                                                          \
        patomic_assert(patomic_is_valid_store_order(order));                   \
        patomic_il_store_##width(obj, desired, order);                         \
    }

#define PATOMIC_DEFINE_LOAD_OPS(width, name, order, vis)                       \
    static PATOMIC_FORCE_INLINE void patomic_opimpl_load_##name(               \
        const volatile void * obj vis(_, int order), void * ret)               \
    {                                                                          \
        patomic_assert(patomic_is_valid_load_order(order));                    \
        patomic_il_load_##width(obj, order, ret);                              \
    }
...
#define PATOMIC_DEFINE_XCHG_OPS_CREATE(width, name, order, vis, inv, opsk)     \
    static PATOMIC_FORCE_INLINE void patomic_opimpl_exchange_##name(           \
        volatile void * obj, const void * desired vis(_, int order),           \
        void * ret)                                                            \
    {                                                                          \
        patomic_assert(patomic_is_valid_order(order));                         \
        patomic_il_exchange_##width(obj, desired, order, ret);                 \
    }                                                                          \
    static PATOMIC_FORCE_INLINE int patomic_opimpl_cmpxchg_##name(             \
        volatile void * obj, void * expected,                                  \
        const void * desired vis(_, int succ) vis(_, int fail))                \
    {                                                                          \
        inv(int succ = order;)                                                 \
        ...
        return patomic_il_compare_exchange_##width(obj, expected, desired,     \
                                                   succ, fail);                \
    }                                                                          \
...
#define PATOMIC_DEFINE_BIT_TEST_OPS(width, name, order, vis)                   \
    static PATOMIC_FORCE_INLINE int patomic_opimpl_test_##name(                \
        const volatile void * obj, int offset vis(_, int order))               \
    {                                                                          \
        /* declarations */                                                     \
        ...
        /* op and return */                                                    \
        patomic_il_load_##width(obj, order, &val);                             \
...
#define PATOMIC_DEFINE_BIT_TEST_MODIFY_OPS(width, name, order, vis)            \
    static PATOMIC_FORCE_INLINE int patomic_opimpl_test_compl_##name(          \
        volatile void * obj, int offset vis(_, int order))                     \
    {                                                                          \
        /* declarations */                                                     \
    ...
    static PATOMIC_FORCE_INLINE int patomic_opimpl_test_set_##name(            \
        volatile void * obj, int offset vis(_, int order))                     \
    {                                                                          \
        patomic_assert(offset >= 0);                                           \
        patomic_assert(width > offset);                                        \
        patomic_assert(patomic_is_valid_order(order));                         \
        return patomic_il_bit_test_and_set_##width(obj, offset, order);        \
    }                                                                          \
    static PATOMIC_FORCE_INLINE int patomic_opimpl_test_reset_##name(          \
        volatile void * obj, int offset vis(_, int order))                     \
    {                                                                          \
        patomic_assert(offset >= 0);                                           \
        patomic_assert(width > offset);                                        \
        patomic_assert(patomic_is_valid_order(order));                         \
        return patomic_il_bit_test_and_reset_##width(obj, offset, order);      \
    }
...
#define PATOMIC_DEFINE_BIN_OPS_CREATE(width, name, order, vis, opsk)           \
    static PATOMIC_FORCE_INLINE void patomic_opimpl_fetch_or_##name(           \
        volatile void * obj, const void * arg vis(_, int order), void * ret)   \
    {                                                                          \
        patomic_assert(patomic_is_valid_order(order));                         \
        patomic_il_or_##width(obj, arg, order, ret);                           \
    }                                                                          \
    static PATOMIC_FORCE_INLINE void patomic_opimpl_or_##name(                 \
        volatile void * obj, const void * arg vis(_, int order))               \
    {                                                                          \
        patomic_uint##width##_t val;                                           \
        /* assertion in called function */                                     \
        patomic_opimpl_fetch_or_##name(obj, arg vis(_, order), &val);          \
    }                                                                          \
    static PATOMIC_FORCE_INLINE void patomic_opimpl_fetch_xor_##name(          \
        volatile void * obj, const void * arg vis(_, int order), void * ret)   \
    {                                                                          \
        patomic_assert(patomic_is_valid_order(order));                         \
        patomic_il_xor_##width(obj, arg, order, ret);                          \
    }                                                                          \
    static PATOMIC_FORCE_INLINE void patomic_opimpl_xor_##name(                \
        volatile void * obj, const void * arg vis(_, int order))               \
    {                                                                          \
        patomic_uint##width##_t val;                                           \
        /* assertion in called function */                                     \
        patomic_opimpl_fetch_xor_##name(obj, arg vis(_, order), &val);         \
    }                                                                          \
    static PATOMIC_FORCE_INLINE void patomic_opimpl_fetch_and_##name(          \
        volatile void * obj, const void * arg vis(_, int order), void * ret)   \
    {                                                                          \
        patomic_assert(patomic_is_valid_order(order));                         \
        patomic_il_and_##width(obj, arg, order, ret);                          \
    }                                                                          \
    static PATOMIC_FORCE_INLINE void patomic_opimpl_and_##name(                \
        volatile void * obj, const void * arg vis(_, int order))               \
    {                                                                          \
        patomic_uint##width##_t val;                                           \
        /* assertion in called function */                                     \
        patomic_opimpl_fetch_and_##name(obj, arg vis(_, order), &val);         \
    }                                                                          \
    static PATOMIC_FORCE_INLINE void patomic_opimpl_fetch_not_##name(          \
        volatile void * obj vis(_, int order), void * ret)                     \
    {                                                                          \
        /* declarations */                                                     \
        patomic_uint##width##_t expected;                                      \
        patomic_uint##width##_t desired;                                       \
    ...
    static PATOMIC_FORCE_INLINE void patomic_opimpl_not_##name(                \
        volatile void * obj vis(_, int order))                                 \
    {                                                                          \
        patomic_uint##width##_t val;                                           \
        /* assertion in called function */                                     \
        patomic_opimpl_fetch_not_##name(obj vis(_, order), &val);              \
    }                                                                          \
...
#define PATOMIC_DEFINE_ARI_OPS_CREATE(width, name, order, vis, opsk)           \
    static PATOMIC_FORCE_INLINE void patomic_opimpl_fetch_add_##name(          \
        volatile void * obj, const void * arg vis(_, int order), void * ret)   \
    {                                                                          \
        patomic_assert(patomic_is_valid_order(order));                         \
        patomic_il_exchange_add_##width(obj, arg, order, ret);                 \
    }                                                                          \
    static PATOMIC_FORCE_INLINE void patomic_opimpl_add_##name(                \
        volatile void * obj, const void * arg vis(_, int order))               \
    {                                                                          \
        patomic_uint##width##_t val;                                           \
        /* assertion in called function */                                     \
        patomic_opimpl_fetch_add_##name(obj, arg vis(_, order), &val);         \
    }                                                                          \
    static PATOMIC_FORCE_INLINE void patomic_opimpl_fetch_sub_##name(          \
        volatile void * obj, const void * arg vis(_, int order), void * ret)   \
    {                                                                          \
        ...
        patomic_il_exchange_add_##width(obj, &arg_val, order, ret);            \
    }                                                                          \
    static PATOMIC_FORCE_INLINE void patomic_opimpl_sub_##name(                \
        volatile void * obj, const void * arg vis(_, int order))               \
    {                                                                          \
        patomic_uint##width##_t val;                                           \
        /* assertion in called function */                                     \
        patomic_opimpl_fetch_sub_##name(obj, arg vis(_, order), &val);         \
    }                                                                          \
    static PATOMIC_FORCE_INLINE void patomic_opimpl_fetch_inc_##name(          \
        volatile void * obj vis(_, int order), void * ret)                     \
    {                                                                          \
        patomic_assert(patomic_is_valid_order(order));                         \
        patomic_il_increment_##width(obj, order, ret);                         \
    }                                                                          \
    static PATOMIC_FORCE_INLINE void patomic_opimpl_inc_##name(                \
        volatile void * obj vis(_, int order))                                 \
    {                                                                          \
        patomic_uint##width##_t val;                                           \
        /* assertion in called function */                                     \
        patomic_opimpl_fetch_inc_##name(obj vis(_, order), &val);              \
    }                                                                          \
    static PATOMIC_FORCE_INLINE void patomic_opimpl_fetch_dec_##name(          \
        volatile void * obj vis(_, int order), void * ret)                     \
    {                                                                          \
        patomic_assert(patomic_is_valid_order(order));                         \
        patomic_il_decrement_##width(obj, order, ret);                         \
    }                                                                          \
    static PATOMIC_FORCE_INLINE void patomic_opimpl_dec_##name(                \
        volatile void * obj vis(_, int order))                                 \
    {                                                                          \
        patomic_uint##width##_t val;                                           \
        /* assertion in called function */                                     \
        patomic_opimpl_fetch_dec_##name(obj vis(_, order), &val);              \
    }                                                                          \
    static PATOMIC_FORCE_INLINE void patomic_opimpl_fetch_neg_##name(          \
        volatile void * obj vis(_, int order), void * ret)                     \
    {                                                                          \
        /* declarations */                                                     \
        patomic_uint##width##_t expected;                                      \
    ...
    static PATOMIC_FORCE_INLINE void patomic_opimpl_neg_##name(                \
        volatile void * obj vis(_, int order))                                 \
    {                                                                          \
        patomic_uint##width##_t val;                                           \
        /* assertion in called function */                                     \
        patomic_opimpl_fetch_neg_##name(obj vis(_, order), &val);              \
    }                                                                          \
...
#define PATOMIC_DEFINE_OPS_CREATE_ALL(width)                                   \
    PATOMIC_DEFINE_OPS_CREATE_CA(width, width##_acquire, patomic_ACQUIRE,      \
                                 HIDE_P, SHOW, ops)                            \
    PATOMIC_DEFINE_OPS_CREATE_R(width, width##_release, patomic_RELEASE,       \
                                HIDE_P, SHOW, ops)                             \
    PATOMIC_DEFINE_OPS_CREATE_AR(width, width##_acq_rel, patomic_ACQ_REL,      \
                                 HIDE_P, SHOW, ops)                            \
    PATOMIC_DEFINE_OPS_CREATE_RSC(width, width##_relaxed, patomic_RELAXED,     \
                                  HIDE_P, SHOW, ops)                           \
    PATOMIC_DEFINE_OPS_CREATE_RSC(width, width##_seq_cst, patomic_SEQ_CST,     \
                                  HIDE_P, SHOW, ops)                           \
    /* all ops are in RSC */                                                   \
    PATOMIC_DEFINE_OPS_CREATE_RSC(width, width##_explicit, order, SHOW_P,      \
                                  HIDE, ops_explicit)
...
#define PATOMIC_SET_RET(width, order, ret)                                     \
    {                                                                          \
        ...
        case patomic_RELAXED:                                                  \
            (ret) = patomic_ops_create_##width##_relaxed();                    \
            break;                                                             \
        case patomic_CONSUME:                                                  \
        case patomic_ACQUIRE:                                                  \
            (ret) = patomic_ops_create_##width##_acquire();                    \
            break;                                                             \
        case patomic_RELEASE:                                                  \
            (ret) = patomic_ops_create_##width##_release();                    \
            break;                                                             \
        case patomic_ACQ_REL:                                                  \
            (ret) = patomic_ops_create_##width##_acq_rel();                    \
            break;                                                             \
        case patomic_SEQ_CST:                                                  \
            (ret) = patomic_ops_create_##width##_seq_cst();                    \
            break;                                                             \
...
patomic_t
patomic_impl_create_msvc(size_t byte_width, patomic_memory_order_t order,
...
patomic_explicit_t
patomic_impl_create_explicit_msvc(size_t byte_width, unsigned int options)
{
...
patomic_impl_create_transaction_msvc(unsigned int options)
{
