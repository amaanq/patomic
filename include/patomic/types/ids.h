...
extern "C"
{
     ...
     * - each id must have a unique value which is a positive power of 2, except
     * for NULL which has the value 0
     ...
     *
     * - unsigned long macros are used here rather than an enum to extend the
     * number of implementations we can support (up to 32)
     * - ids_ALL is all bits set rather than other values combined to avoid an
     * ABI break if a new id is added
     */
...
#define patomic_ids_ALL (~0ul)
     ...
     * - OS:   implementation provided by host OS, e.g. Interlocked,
     * atomic_ops(3C)
     ...
     *         certain circumstances, e.g. __atomic -> libatomic for 16B
     * operations most compiler builtins and stdatomic.h macros fall in this
     * category
     ...
     *   assumption that an inline ASM implementation will have the least
     * overhead)
     * - values are NOT ordered on quality of implementation (i.e. on x86 an ASM
     *   implementation of add is free to use a LOCK CMPXCHG loop over LOCK
     * XADD)
     *
     * Note:
     ...
     *   provide an ASM implementation, otherwise, LIB, OS, or DYN should be
     * used
     */
    ...
    typedef enum
    {
        ...
        patomic_kinds_ALL = patomic_kind_DYN | patomic_kind_OS |
                            patomic_kind_LIB | patomic_kind_BLTN |
                            patomic_kind_ASM
    } patomic_kind_t;
    ...
    PATOMIC_EXPORT unsigned long patomic_get_ids(unsigned int kinds);

    /* returns kind for given id, or kind_UNKN for invalid id or id_NULL */
    PATOMIC_EXPORT unsigned int patomic_get_kind(unsigned long id);
