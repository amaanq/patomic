...
extern "C"
{
     ...
     * TRANSACTION FLAG
     *
     * - used to trigger an abort in a live transaction if modified, by reading
     * from it at the start of each transaction
     * - any modification to any memory in the same cache line should cause an
     * abort
     * - a patomic_transaction_flag_t pointer can point to any memory location
     * since it's just a volatile unsigned char *
     * - traditionally this may point to a global lock so that transactions can
     * be used safely alongside regular locks
     */
    ...
    /* WARNING: see warning for PATOMIC_MAX_CACHE_LINE_SIZE_ABI_UNSTABLE in
     * align.h
     */
    typedef struct
    {
        unsigned char
            _padding_pre[PATOMIC_MAX_CACHE_LINE_SIZE_ABI_UNSTABLE - 1];
        patomic_transaction_flag_t flag;
     ...
     * TRANSACTION CMPXCHG
     *
     * - used in double_cmpxchg and multi_cmpxchg to pass multiple memory
     * locations
     ...
     *
     * - width is expected to be non-zero, but zero is still a valid value (it
     * just isn't explicitly checked for to skip steps in an op)
     * - width is ignored in generic transaction functions
     *
     * - flag and fallback_flag may point to the same object, or be NULL (in
     ...
     * - it may be useful to use flag as a unique read-write lock, and
     * fallback_flag as a shared read-only lock
     *
     * - wfb: with fallback
     */
     ...
     *   shift execution to the fallback path, regardless of whether all
     * attempts on the primary path have been exhausted
     ...
     *   operation
     * - attempts_made: how many attempts were made to commit the atomic
     * operation
     * - fallback_attempts_made: how many attempts were made to commit the
     * fallback atomic operation
     *
     * - status will default to TSUCCESS if attempts_made is 0 because attempts
     * was set to 0 in config
     ...
     *
     * - max_rmw_memory (N): test transaction performs equivalent to a
     * successful fp_cmpxchg_weak with a width of N, with N being the maximum
     * value tested where the transaction eventually succeeded
     * - max_load_memory (N): test transaction performs equivalent to fp_load
     * with a width of N, with N being the maximum value tested where the
     * transaction eventually succeeded
     *
     * - min_rmw_attempts: the number of attempts taken for the max_rmw_memory
     ...
     * - they represent the best possible outcome, which may not be achievable
     ...
     * - the string.h versions may use instructions which may cause a
     * transactional abort (e.g. vzeroupper on x86_64 in memcmp)
     * - these counterparts are guaranteed to not cause an abort due to the
     * usage of such instructions (although they may still cause an abort for
     * other reasons, such as accessing too much memory)
     * - these counterparts will typically be significantly faster than a
     * volatile char loop
     *
     ...
     *
     * WARNING:
     * - these functions are NOT atomic (which is why they're here and not in
     * ops.h)
