...
#define PATOMIC_MAX_CACHE_LINE_SIZE_ABI_UNSTABLE 128
    ...
    PATOMIC_EXPORT size_t patomic_cache_line_size(void);
     ...
     *
     * - in meets_minimum, if width == 0, then align.size_within value is
     * ignored
     * - this is the same as checking that ptr is aligned to both align.minimum
     *   and align.size_within
     *
     * NOTE:
     * - the fact that a call to one of these functions succeeds, does not mean
     * that align.recommended or align.minimum is a valid alignment for an
     * object in C
     * - it only means that ops functions can accept objects with such an
     * alignment
     * - e.g. align.recommended == 0x3 means that ops functions could accept a
     *   pointer to an object at address 0x3, not that 0x3 is a valid alignment
     * in C (it isn't)
     */

    PATOMIC_EXPORT int
    patomic_align_meets_recommended(const volatile void * ptr,
                                    patomic_align_t align);

    PATOMIC_EXPORT int patomic_align_meets_minimum(const volatile void * ptr,
                                                   patomic_align_t align,
                                                   size_t width);
