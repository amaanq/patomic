...
extern "C"
{
    ...
    typedef enum
    {
        ...
        patomic_opcats_IMPLICIT = patomic_opcat_LDST | patomic_opcat_XCHG |
                                  patomic_opcat_BIT | patomic_opcats_BIN |
                                  patomic_opcats_ARI,
        patomic_opcats_EXPLICIT = patomic_opcats_IMPLICIT,
        patomic_opcats_TRANSACTION = patomic_opcats_EXPLICIT |
                                     patomic_opcat_SPEC | patomic_opcat_FLAG |
                                     patomic_opcat_RAW
    } patomic_opcat_t;
     ...
     * - there are no fetch opkinds; instead use the regular opkind version with
     * a fetch opcat (_F)
     *
     * Note:
    ...
    typedef enum
    {
        ...
        patomic_opkinds_BIT = patomic_opkind_TEST | patomic_opkind_TEST_SET |
        ...
        patomic_opkinds_BIN = patomic_opkind_OR | patomic_opkind_XOR |
                              patomic_opkind_AND | patomic_opkind_NOT
        /* arithmetic */
        ,
        ...
        patomic_opkinds_ARI = patomic_opkind_ADD | patomic_opkind_SUB |
                              patomic_opkind_INC | patomic_opkind_DEC |
                              patomic_opkind_NEG
        /* special */
        ,
        ...
        patomic_opkinds_SPEC =
            patomic_opkind_DOUBLE_CMPXCHG | patomic_opkind_MULTI_CMPXCHG |
            patomic_opkind_GENERIC | patomic_opkind_GENERIC_WFB
        /* raw */
        ,
        ...
        patomic_opkinds_RAW = patomic_opkind_TBEGIN | patomic_opkind_TABORT |
                              patomic_opkind_TCOMMIT | patomic_opkind_TTEST
    } patomic_opkind_t;
     ...
     *   operations supported, and returns the result (i.e. 0 if everything in
     * each set category is supported)
     * - invalid categories are ignored and remain set
     */

    PATOMIC_EXPORT unsigned int
    patomic_feature_check_all(const patomic_ops_t * ops, unsigned int opcats);

    PATOMIC_EXPORT unsigned int
    patomic_feature_check_all_explicit(const patomic_ops_explicit_t * ops,
                                       unsigned int opcats);

    PATOMIC_EXPORT unsigned int
    patomic_feature_check_all_transaction(const patomic_ops_transaction_t * ops,
                                          unsigned int opcats);
     ...
     * - opcats: one or more opcat_t flags combined
     *
     * - checks if any of the operations in each of the categories set in opcats
     * is supported
     * - unsets all the categories set in opcats which don't have at least one
     * of their operations supported, and returns the result (i.e. 0 if at least
     * one operation in each set category is supported)
     * - invalid categories are ignored and remain set
     */

    PATOMIC_EXPORT unsigned int
    patomic_feature_check_any(const patomic_ops_t * ops, unsigned int opcats);

    PATOMIC_EXPORT unsigned int
    patomic_feature_check_any_explicit(const patomic_ops_explicit_t * ops,
                                       unsigned int opcats);

    PATOMIC_EXPORT unsigned int
    patomic_feature_check_any_transaction(const patomic_ops_transaction_t * ops,
                                          unsigned int opcats);
     ...
     *
     * - unsets all the operations set in opkinds which aren't available in the
     * set opcat, and returns the result (i.e. 0 if all the operations in the
    ...
    PATOMIC_EXPORT unsigned int
    patomic_feature_check_leaf(const patomic_ops_t * ops, unsigned int opcat,
                               unsigned int opkinds);

    PATOMIC_EXPORT unsigned int
    patomic_feature_check_leaf_explicit(const patomic_ops_explicit_t * ops,
                                        unsigned int opcat,
                                        unsigned int opkinds);

    PATOMIC_EXPORT unsigned int patomic_feature_check_leaf_transaction(
        const patomic_ops_transaction_t * ops, unsigned int opcat,
        unsigned int opkinds);
