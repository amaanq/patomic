...
extern "C"
{
     ...
     * - if it is, .align may also be modified to meet the requirements for all
     * the operations now present
     * - therefore it is advisable to sort structs by .align when combining more
     *   than two, in order to end up with the least restrictive values for
     * .align
     *
     * - transaction structs are not combinable
     * - a transaction provides .sstring which cannot be combined with other
     *   implementations
     * - additionally, it is deemed fairly unlikely that any platform will have
     * more than a single transaction implementation
     */

    PATOMIC_EXPORT void patomic_combine(patomic_t * priority,
                                        const patomic_t * other);

    PATOMIC_EXPORT void
    patomic_combine_explicit(patomic_explicit_t * priority,
     ...
     *
     * - for transaction, the implementation with the most efficient kind will
     * be returned that supports at least one op (from the selected
     * implementations)
     * - if multiple implementations fulfil these requirements, it is
     * unspecified which one will be returned
     *
     * - in both cases, if no implementation exists matching both kinds and ids,
     * the NULL implementation is returned
     */

    PATOMIC_EXPORT patomic_t patomic_create(size_t byte_width,
    ...
    patomic_create_explicit(size_t byte_width, unsigned int opts,
                            unsigned int kinds, unsigned long ids);

    PATOMIC_EXPORT patomic_transaction_t patomic_create_transaction(
        unsigned int opts, unsigned int kinds, unsigned long ids);
