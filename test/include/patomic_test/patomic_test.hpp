...
template <size_t N>
using order_array = std::array<patomic_memory_order_t, N>;
template <size_t N>
using id_array = std::array<patomic_id_t, N>;

static constexpr auto
get_ids() -> id_array<5>
{
    return {patomic_id_NULL, patomic_id_MSVC, patomic_id_STD, patomic_id_TSX,
            patomic_id_GNU};
}

static constexpr auto
get_id_name(patomic_id_t id) -> const char *
{
    switch(id)
    {
...
static constexpr auto
get_orders() -> order_array<6>
{
    return {patomic_RELAXED, patomic_CONSUME, patomic_ACQUIRE,
            patomic_RELEASE, patomic_ACQ_REL, patomic_SEQ_CST};
}

static constexpr auto
get_order_name(patomic_memory_order_t order) -> const char *
{
    switch(order)
    {
...
static auto
get_widths() -> std::vector<size_t>
{
    // widths in bytes
    static const auto widths = std::vector<size_t>{1, 2, 4, 8, 16};
    return widths;
}
