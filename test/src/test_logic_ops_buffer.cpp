...
#define CURRY_OP_NO_RET(name, parent)                                          \
    ::patomic::test::curry_op_no_ret(this->m_i##parent##name,                  \
                                     this->m_e##parent##name,                  \
                                     this->m_is_explicit, this->m_order)

#define CURRY_OP_RET(name, parent)                                             \
    ::patomic::test::curry_op_ret(this->m_i##parent##name,                     \
                                  this->m_e##parent##name,                     \
                                  this->m_is_explicit, this->m_order)

#define CURRY_OP_CMPXCHG(name, parent)                                         \
    ::patomic::test::curry_op_cmpxchg(this->m_i##parent##name,                 \
                                      this->m_e##parent##name,                 \
                                      this->m_is_explicit, this->m_order)
...
TEST_P(BufferOpsLogicTestFixture, fp_store)
{
    ...
    // skip
    if(!patomic_is_valid_store_order(m_order))
    {
        GTEST_SKIP_("Invalid memory order");
    }
    else if(fp_store == nullptr)
    {
...
TEST_P(BufferOpsLogicTestFixture, fp_load)
{
    ...
    // skip
    if(!patomic_is_valid_load_order(m_order))
    {
        GTEST_SKIP_("Invalid memory order");
    }
    else if(fp_load == nullptr)
    {
        GTEST_SKIP_("Not implemented");
    }
    // test
...
INSTANTIATE_TEST_SUITE_P(BufferOpsLogicTest, BufferOpsLogicTestFixture,
